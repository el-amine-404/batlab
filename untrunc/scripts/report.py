...
from catalog import packet_only_note
...
STATUS_MEANING = {
    'decode-errors': 'decodes with errors (damaged inside, or partly unsupported)',
    'unreadable-or-no-video': 'cannot be opened as video (often a truncated recording; '
                              'permissions or an unsupported format can also cause this)',
}
BATCH_LABELS = {
...
def suspect_detail(item, log_dir):
    if item.get('status') == 'decode-errors':
        detail = f'{item.get("error_log_lines", "?")} decoder message(s)'
        if item.get('log'):
            first = first_line_of_file(Path(log_dir) / Path(item['log']).name)
            if first:
...
def render_suspects(catalog, catalog_file, src, top, warnings=()):
    ...
    lines += ['', f'Suspects ({len(found)}); * marks the matches a repair would use (max_references = {top}):']
    for number, item in enumerate(found, 1):
        lines += ['', f'{number:>3}. {item["path"]}',
                  f'     {item["status"]} · {human_size(item.get("size"))}',
                  f'     {suspect_detail(item, Path(catalog_file).parent)}']
    ...
    lines += [f'  {status}: {STATUS_MEANING[status]}' for status in SUSPECT_STATUSES if any(i['status'] == status for i in found)]
    lines.append('Matches are ranked hypotheses, not proof of the same camera; review every result.')
    lines.append('Next: repair one file by setting "broken" and running `make untrunc-case-run`,')
    lines.append('      or repair every suspect with `make untrunc-case-batch`.')
    return lines


def render_batch(entries, report_file, remaining=0):
